...
**Note:** the _unused_ shadcn `ui/progress.tsx` scaffold already sitting in the web repo (Radix-based, `h-2`/`rounded-full`/`bg-primary/20` defaults) was deliberately **not** used as the template — it's dead code the real app pages never render. Base styling instead mirrors the actual, real usage pattern quoted in the Goal above. The public API (`value` + `className` passthrough) still follows the general shadcn/RNR shape for consistency with the rest of `ui/`.
...
**Real verification limitation (disclosed, not glossed over):** `react-native-webview` has **no web platform implementation at all** — confirmed by inspecting the installed package (no `.web.*` files, no `"web"` field in its `package.json`). Empirically confirmed too: wiring `VimeoEmbed` into a temporary test screen bundled cleanly (0 errors/warnings) on the web dev server, but rendered no `<iframe>` and no reference to `player.vimeo.com` at all — it silently no-ops on web rather than crashing. This means **actual video playback cannot be verified in this environment at all, on web or native** (no simulator/device available) — this is exactly what the task's own manual testing checklist anticipated ("a real device or simulator... may require a dev-client rebuild, not just Expo Go"). What _was_ verified: the component's prop names and URL construction are correct per the library's real types, and it doesn't break the app's module graph.
...
### Task 8 — Expose `aiChat` as a plain HTTP endpoint _(touches the web/server project)_ — ✅ Completed
...
**Files modified** _(in `kitchen-haven-club`)_:
...
- RHF's `defaultValues: settings` snapshots the persisted settings once on mount — this alone reproduces the web's draft/commit-on-save behavior with no extra bookkeeping: an unsaved edit lives only in RHF's in-memory form state, so navigating away unmounts the screen and discards it; navigating back re-mounts and re-reads the last **saved** `settings` value. No separate `draft` `useState` (unlike the web) is needed since RHF's form state already _is_ the draft, the same reasoning Task 11 (Login) already established for RHF-managed screens.
...
- [ ] Edit a field, navigate away _without_ saving, come back — confirm the unsaved edit was discarded — **not independently verified interactively**; holds by construction (RHF's defaultValues re-snapshot the persisted value on every fresh mount), not exercised on a live remount cycle.
...
_(Note: extended shortly after to also cover videos, at the user's direct request — see "Post-Task-26 extension" below. The notes below describe the screen as first shipped, recipes-only.)_
...
_(Note: extended shortly after to also cover viewed recipes, at the user's direct request — see "Post-Task-27 extension" below. The notes below describe the screen as first shipped, videos-only.)_
...
- Rewrote `src/app/app/profile/faq.tsx` (real screen, replacing Task 1's stub).
  ...
  **Acceptance criteria:** All FAQ items from `mock-data.faqItems` present; only one open at a time (matches web). — Structurally satisfied: `faqItems.map(...)` renders every item; `type="single"` enforces single-open. **Verification:** `npx tsc --noEmit` is clean (only the 2 long-standing pre-existing `global.css`/`animated-icon.module.css` side-effect-import errors remain, unrelated). The dev-server SSR check that Tasks 1-28 used could **not** be run this session: this machine is missing the MSVC 2015-2022 runtime (`vcruntime140.dll` et al.), so `lightningcss`'s native module fails to load and Metro's NativeWind pipeline won't start (blocks `expo export`/`expo start` alike). Built against already-verified patterns (SafeAreaView header from Settings/Recipes, `GradientView`/`Icon`/`Linking` all proven elsewhere). An on-device pass is still needed.
  **Manual testing checklist:**
...
- Rewrote `src/app/app/profile/tips.tsx` (real screen, replacing Task 1's stub).
  ...
  **Acceptance criteria:** Category filter matches web's category list exactly — same 7 labels, same `cat === "Tout" || a.category === cat` predicate, ported as-is. **Verification:** `npx tsc --noEmit` clean (same 2 unrelated pre-existing errors). SSR check blocked by the missing MSVC runtime (see Task 29); built against already-verified patterns (Recipes/Tutorials ToggleGroup, expo-image wrapper). On-device pass still needed.
  **Manual testing checklist:**
...
**Suggested commit:** _(n/a — not built; see Task 28)_
...
- Mounted globally by wrapping `app/_layout.tsx`'s `<Tabs>` in a `flex-1` `<View>` and rendering `<AIChat />` as a sibling after it, so the FAB's `absolute bottom-44 right-4` positioning (matching web's `fixed bottom-44 right-4`) floats over every `/app/*` screen — the web mounts `<AIChat />` globally in `MobileShell` the same way. `bottom-44` sits it above both the in-flow bottom nav and the per-page Notes FAB (`bottom-6`), matching the web's own stacking.
  **Keyboard-overlay fix (real-device bug, reported by user after first delivery):** on the first cut the soft keyboard covered the chat input. Root cause: the shared `DialogOverlay` wraps content in react-native-screens' `FullWindowOverlay` on iOS — a _separate native window_ that `KeyboardAvoidingView` cannot measure the keyboard against, so the input stayed hidden. Fixed by giving the chat sheet its own self-contained overlay inside `DialogPortal` (a `bg-black/50` `Pressable` backdrop + a `KeyboardAvoidingView` wrapping `DialogPrimitive.Content`), deliberately **not** the shared `DialogOverlay`, so it renders in the app's own window where keyboard avoidance works: iOS uses `behavior="padding"` to lift the sheet above the keyboard; Android is left to the OS default `adjustResize` (Expo's default) so no double-shift. `pointerEvents: "box-none"` on the wrapper (via `style`, not the deprecated prop) lets taps in the empty area above the sheet fall through to the backdrop to close. `DialogPortal` still gates the whole thing on `open` (confirmed in the primitive source), and `DialogPrimitive.Content` still registers the Android hardware-back-to-close handler. (Note: `AddNoteButton`'s note dialog still uses the `FullWindowOverlay`-wrapping `DialogContent`; its textarea sits near the top so the keyboard is less likely to cover it, but it could get the same treatment if it turns out to be a problem on-device.)
  **Files modified:**
...
- Modified `src/app/app/_layout.tsx` (wrapped `<Tabs>` in a `flex-1` `View`, mounted `<AIChat />` globally).
  ...
  **Acceptance criteria:** Sending a message shows a loading state, then either the assistant's reply (with working recipe/video reference chips) or the matching French error copy on failure. — Structurally satisfied: `loading` drives the spinner bubble + disabled send button; `send()` is a verbatim port of the web's try/catch, and `aiChat()` (Task 9, already verified in isolation across 5 scenarios) returns the same `{ok,...}` union whose error string gets the `😔 ` prefix at render time. **Verification:** `npx tsc --noEmit` clean (same 2 unrelated pre-existing errors), including after the keyboard-overlay fix. SSR check blocked by the missing MSVC runtime (see Task 29), and the endpoint is still the `.example.com` placeholder (Task 8 never deployed), so a real round-trip can't be exercised regardless. Built against already-verified patterns (Dialog primitives from `AddNoteButton`, `GradientView`/`Icon`/typed-route `Link`, `aiChat()` from Task 9). On-device pass (and a real deployed endpoint) still needed — the keyboard fix in particular is native-runtime behavior that only a device can confirm.
  **Manual testing checklist:**
...
- [ ] Confirm every `Link`/`router.push` in the app resolves (no "unmatched route" screens).
      **Suggested commit:** `chore: full regression pass fixes` _(or split per bug found)_

---

## v2 client rebrand (2026-07-05) — design-system update

The client sent real design assets (`assets/new-assets/`: `auth-page.png`, `dashboard-page.png`, `index-page.png`, `new-hero-image.png`, `ogee-arch.svg`, `perle-des-lys-log-no-text.svg`, `perle-des-lys-log-with-text.svg`) requesting UI/color changes and a fix so the bottom nav ("now glued to the bottom") doesn't hide page content. The mockups turned out to show more than a palette swap — a new arch logo, a script wordmark ("Perledeslys" replacing "PERLEDESLYS"), the founder renamed "Ghania" (not "Lys"), a redesigned Login screen, and restyled calendar/video cards — confirmed with the user before proceeding to a full match rather than guessing scope.
...
- [x] Task 29 — FAQ screen (RNR Accordion, single-open; `tsc` clean; on-device pass still pending, see task notes)
- [x] Task 30 — Tips screen (ToggleGroup category filter; "Lys"→"Ghania" copy rename; `tsc` clean; on-device pass still pending)
...
- [x] Task 32 — AI Chat (global floating overlay, RNR Dialog primitives, verbatim parser port; keyboard-overlay bug fixed; `tsc` clean; endpoint still undeployed + on-device pass pending, see task notes)
