...
// A single recipe card. Memoized so scrolling / filtering doesn't re-render
// every card — only cards whose recipe reference changed.
const RecipeCard = memo(function RecipeCard({ recipe: r }: { recipe: Recipe }) {
  return (
    <Link
      href={{ pathname: "/app/recipes/[recipeId]", params: { recipeId: r.id } }}
      asChild
    >
      <Pressable style={{ width: "48%", marginBottom: 12 }}>
