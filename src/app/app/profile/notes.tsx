...
import type { Note } from "@/types/local-store";
...
// Notes are unbounded, device-local data (no backend involved) — this window
// size is purely a rendering-cost control, raised as the user scrolls.
const PAGE_SIZE = 20;
...
const NoteCard = memo(function NoteCard({
  ...
  return (
    <View className="mx-5 mb-3 rounded-2xl border border-border bg-card p-4">
      <View className="flex-row items-center justify-between">
        <Text className="text-[10px] font-medium uppercase tracking-wider text-primary">
          {formatDate(n.createdAt)}
        </Text>
        <Pressable onPress={() => onRemove(n.id)} accessibilityLabel="Supprimer">
...
export default function NotesScreen() {
  ...
  return (
    <SafeAreaView className="flex-1 bg-background" edges={["top"]}>
      <FlatList
        data={shown}
        renderItem={({ item }) => <NoteCard note={item} onRemove={remove} />}
        keyExtractor={(n) => n.id}
        ...
        onEndReached={() => setVisible((v) => Math.min(v + PAGE_SIZE, notes.length))}
